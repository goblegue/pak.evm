...
VoteRecord *VoteRepository::getAllVotesForAudit(int &size)
{
    ...
    while (q.next())
    {
        arr[i].setId(q.value(0).toInt());
        arr[i].setCandidateCnic(q.value(1).toString());
        arr[i].setTimestamp(q.value(2).toString());
        arr[i].setCurrentHash(q.value(3).toByteArray());
        i++;
    }
    return arr;
}
