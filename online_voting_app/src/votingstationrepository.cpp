...
VotingStation* votingstationrepository::getAllActiveStations(int &vSSize) {
    ...
    int i = 0;
    for (auto&& doc : cursor) {
        stations[i].setId(QString::fromStdString(doc["id"].get_string().value.to_string()));
        stations[i].setStationName(QString::fromStdString(doc["stationName"].get_string().value.to_string()));
        stations[i].setCity(QString::fromStdString(doc["city"].get_string().value.to_string()));
        stations[i].setAddress(QString::fromStdString(doc["address"].get_string().value.to_string()));
