...
bool adminrepository::insertAdmin(const Admin &admin) {
    try {
        ...
        builder << "cnic" << admin.getCnic().toStdString()
                << "name" << admin.getName().toStdString()
                << "email" << admin.getEmail().toStdString()
                << "id" << admin.getId().toStdString()
                << "isEmailVerified" << admin.isEmailVerified()
                << "status" << static_cast<int>(admin.getStatus()) // Store Enum as Int
                << "salt" << admin.getSalt();
...
std::optional<Admin> adminrepository::getAdminByEmail(const QString &email) {
    ...
    if (result) {
        ...
        admin.setCnic(QString::fromStdString(view["cnic"].get_string().value.to_string()));
        admin.setName(QString::fromStdString(view["name"].get_string().value.to_string()));
        admin.setEmail(QString::fromStdString(view["email"].get_string().value.to_string()));
        ...
        if (view["statusChangeRequests"] && view["statusChangeRequests"].type() == bsoncxx::type::k_array) {
            ...
            for (auto&& doc : requestsArray) {
                ...
                QString requesterId = QString::fromStdString(reqView["requestById"].get_string().value.to_string());
...
std::optional<Admin> adminrepository::getAdminByCnic(const QString &cnic) {
    ...
    if (result) {
        ...
        admin.setName(QString::fromStdString(view["name"].get_string().value.to_string()));
        admin.setEmail(QString::fromStdString(view["email"].get_string().value.to_string()));
        admin.setId(QString::fromStdString(view["id"].get_string().value.to_string()));
